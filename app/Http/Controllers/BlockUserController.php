...
class BlockUserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            $p_blocked_user_id   = intval($request->get('p_blocked_user_id'));
            $p_flag              = $request->get('p_flag');

            $pdo = \DB::connection()->getPdo();
            // calling stored procedure command

            $sql = 'CALL user_block_pc(@o_status, @o_message, :p_access_token, :p_user_id, :p_blocked_user_id, :p_flag)';
            ...
            $stmt->bindParam(':p_blocked_user_id', $p_blocked_user_id, PDO::PARAM_INT);
            $stmt->bindParam(':p_flag', $p_flag, PDO::PARAM_STR);
