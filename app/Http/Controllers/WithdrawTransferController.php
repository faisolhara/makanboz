...
class WithdrawTransferController extends Controller
{
    protected $now;
    ...
    public function index(Request $request)
    {
        ...
        try {
            $p_withdraw_id             = intval($request->get('p_withdraw_id'));
            $p_transfer_reference      = $request->get('p_transfer_reference');

            $pdo = \DB::connection()->getPdo();
            // calling stored procedure command
            $sql = 'CALL change_pin_pc(@o_status, @o_message, :p_withdraw_id, :p_transfer_reference';

            // prepare for execution of the stored procedure
            $stmt = $pdo->prepare($sql);

            // pass value to the command
            $stmt->bindParam(':p_withdraw_id', $p_withdraw_id, PDO::PARAM_INT);
